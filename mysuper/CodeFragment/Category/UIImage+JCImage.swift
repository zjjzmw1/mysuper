...
extension UIImage {
    ...
    /// 取图片某点像素的颜色
    func colorSwift(atPixel: CGPoint) -> UIColor? {
    ...
    func imageRotated(byDegrees: CGFloat) -> UIImage? {
        return self.imageRotated(byRadians: CGFloat(Double(byDegrees) * .pi / 180));
    }
    ...
    /// - Returns: Image
     func image(text: String, textColor: UIColor, font: UIFont, paragraphStyle: NSParagraphStyle) -> UIImage? {
        ...
        let textSize = text.sizeFor(size: self.size, font: font);
    ...
    class func animatedGIFWithData(data: NSData) -> UIImage {
        ...
        if count <= 1 {
            ...
            for _ in 0...(count - 1) {
                ...
                let frameProperties = cfFrameProperties!;
